...
const TableUsers = () => {
  const deleteUser = async (_id) => {
    if (window.confirm("¿Estás seguro que deseás eliminar este usuario?")) {
      await axios.delete(
        `https://burguercode-db.onrender.com/deleteuser/${_id}`
      );
  ...
  const addUser = async () => {
    try {
      const info = await axios.get(
        "https://burguercode-db.onrender.com/allusers"
      );
      setUser(info.data);
    } catch (error) {
            ...
            <tr className="viewunique">
              ...
              <td>
                <div className="d-flex justify-content-center">
                  <button
                    className="btn btn-danger p-2 m-1 rounded-0 border-0"
                    onClick={(_id) => deleteUser(user._id)}
                    type="button"
                    key={user._id}
                  >
                    <svg
